Feature: Create an offer template
  ...
  Scenario Outline: offer template is created
    Given the server is up
    When an offer template is created with:
      | name   | product_id | contents   | metadata   | period   | trigger   |
      | <name> | <pid>      | <contents> | <metadata> | <period> | <trigger> |
    Then an offer template with name "<name>" exists in game "offer-template-game"

    Examples:
      | name | pid            | contents   | metadata   | period           | trigger                                  |
      | oc1  | com.tfg.sample | { "x": 1 } | { "y": 2 } | { "once": true } | { "from": 1486678078, "to": 1486678079} |
  ...
  Scenario: can't create offer template with same name
    ...
    When an offer template is created with:
      | name | product_id     | contents   | metadata   | period           | trigger                                  |
      | oc2  | com.tfg.sample | { "x": 1 } | { "y": 2 } | { "once": true } | { "from": 1486678078, "to": 1486678079} |
  ...
  Scenario Outline: can't create offer template with invalid payload
    ...
    Examples:
      | name | pid            | contents   | metadata   | period           | trigger                                  | error                                                      |
      |      | com.tfg.sample | { "x": 1 } | { "y": 2 } | { "once": true } | { "from": 1486678078, "to": 1486678079} | The name is required to create a new offer template.       |
      | oc3  |                | { "x": 1 } | { "y": 2 } | { "once": true } | { "from": 1486678078, "to": 1486678079} | The product id is required to create a new offer template. |
      | oc3  | com.tfg.sample |            | { "y": 2 } | { "once": true } | { "from": 1486678078, "to": 1486678079} | The contents are required to create a new offer template.  |
      | oc3  | com.tfg.sample | { "x": 1 } | { "y": 2 } |                   | { "from": 1486678078, "to": 1486678079} | The period is required to create a new offer template.    |
      | oc3  | com.tfg.sample | { "x": 1 } | { "y": 2 } | { "once": true } |                                         | The trigger is required to create a new offer template.     |
